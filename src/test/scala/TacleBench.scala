...
/** Runs the time-predictable integer TACLe kernels (no soft-float, no recursion; the list is in
  * tacle.mk) on all six cores of the wide system at the same time.
  *
  * `make tacle` (src/c/tacle/tacle.mk) links every benchmark once per core window. One Verilator
  * model serves all benchmarks: its program ROM and memLow are loaded by $readmemh from fixed
  * files, rewritten before each simulation. Every core reports its return value and its own
  * instruction and cycle counters for main through its scratchpad (see crt0.S).
  *
  * Results go to build/tacle/results/<benchmark>.csv and are collected in build/tacle/results.csv.
  *
  ...
  */
class TacleBench extends AnyFlatSpec with Matchers with BeforeAndAfterAll {

  val buildDir = new File("build/tacle")
  val progHex = new File(buildDir, "sim/prog.hex")
  val memLowHex = new File(buildDir, "sim/memlow.hex")
  ...
  // the dispatcher's window and six text windows, in 4-word lines
  val romLineBits = bitsFor((7L << windowShift) / 16)
  // six data windows, in words
  val memLowAddrBits = bitsFor((6L << windowShift) / 4)
  ...
  def writeImages(bench: String): Unit = {
    def romLine(addr: Long): Long = (addr & ((1L << (romLineBits + 4)) - 1)) >> 4
    ...
    BattutaSim.writeHex(progHex, (romLine(romBase) -> Util.Binary.load(s"$buildDir/dispatch.text.bin")) +: text, 4)
  ...
  case class Finished(cores: Seq[CoreResult]) extends Outcome
  case class Trapped(cores: Seq[Int], cycle: Int) extends Outcome
  case class TimedOut(cycle: Int) extends Outcome

  def run(dut: WideBattutaArray): Outcome = {
    ...
    while (outcome.isEmpty) {
      ...
      val trap = dut.io.trap.get.peek().litValue
      if (trap != 0) {
        outcome = Some(Trapped((0 until 6).filter(k => trap.testBit(k)), dut.clock.cycle))
      } else if ((0 until 6).forall(ports.inReset)) {
        outcome = Some(Finished((0 until 6).map { k =>
          CoreResult(
            ports.scratchpad(k, 0).toInt,
            ports.scratchpad(k, 1).toLong,
            ports.scratchpad(k, 2).toLong,
            ports.scratchpad(k, 3) == DoneMarker
          )
        }))
      } else if (dut.clock.cycle >= maxCycles) {
        outcome = Some(TimedOut(dut.clock.cycle))
      }
    }
    outcome.get
  }
  ...
  def resultFile(bench: String) = new File(buildDir, s"results/$bench.csv")
  new File(buildDir, "results").mkdirs()

  BattutaSim.filterReporting()

  benchmarks.foreach { bench =>
    "TACLe" should s"run $bench on all six cores" in {
      resultFile(bench).delete()
      assert(new File(buildDir, s"$bench/stamp").exists(), s"$bench did not build, see $makeLog")

      writeImages(bench)
      val sim = model.simulate(s"$buildDir/$bench/sim/".toDir)(run)
  ...
  override def afterAll(): Unit = {
    val out = new PrintWriter(new File(buildDir, "results.csv"))
    try {
