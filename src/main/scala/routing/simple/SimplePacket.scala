...
class SimplePacket[T <: Data](implicit p: SimpleNocParams[T]) extends Bundle {
  ...
  override def toString(): String = {
    if (this.dest.x.isLit && this.dest.y.isLit) {
  ...
  def pretty: String = {
    if (this.dest.x.isLit && this.dest.y.isLit) {
      s"(${dest.x.litValue}, ${dest.y.litValue}) -> ${payload})"
    } else {
      super.toString()
    }
  }
...
object SimplePacket {
  ...
  def Lit[T <: Data](fromto: (Coord, Coord), payload: T)(implicit p: SimpleNocParams[T]): SimplePacket[T] = {
    val (from, to) = fromto
    if (p.wrappedTopology) {
      val dx = (to.x - from.x + p.nx) % p.nx
      val dy = (to.y - from.y + p.ny) % p.ny
      val eastbound = true // dx <= p.nx / 2
      val southbound = true // dy <= p.ny / 2
