...
import routing.RouterDebugPort
import Chisel.debug

object SimpleRouterVerilog extends App {
  implicit val p: SimpleNocParams[UInt] = SimpleNocParams(
    nx = 4,
    ny = 4,
    payloadGen = () => UInt(128.W),
  ...
  emitVerilog(new SimpleRouter(Coord(2, 2)), Array("--target-dir", "eval/slimflit"))
}
...
class SimpleRouter[T <: Data](val coord: Coord)(implicit p: SimpleNocParams[T])
  ...
  val debugPort = if (p.debug) Some(IO(new RouterDebugPort(new SimplePacket))) else None
  ...
  buffedIngresses.foreach { bufOut =>
    if (p.debug) {
      debugPort.get.apply(bufOut.dir).valid := bufOut.valid
      debugPort.get.apply(bufOut.dir).packet := bufOut.bits
    }
  }
  ...
  routingBlocks.foreach { block => 
    if (p.debug) {
      debugPort.get.apply(block.ingressDir).reqs.all.foreach(_ := 0.B)
      block.to.all.foreach { case port =>
        debugPort.get.apply(block.ingressDir).reqs(port.dir) := port.valid
      }
    }
  }
