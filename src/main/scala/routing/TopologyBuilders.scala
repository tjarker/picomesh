...
abstract class TopologyBuilder {
  ...
  def create(routerFactory: RouterFactory): (Seq[RouterPortLike], Seq[Option[RouterDebugPort[_]]])
...
class MeshBuilder(nx: Int, ny: Int) extends TopologyBuilder {
  ...
  def create(routerFactory: RouterFactory): (Seq[RouterPortLike], Seq[Option[RouterDebugPort[_]]]) = {
    ...
    val ports = for (i <- 0 until (nx * ny)) yield routers(i % nx)(i / nx).port(Local)
    val debugPorts = for (i <- 0 until (nx * ny)) yield routers(i % nx)(i / nx).debugPort
    (ports, debugPorts)
  }
